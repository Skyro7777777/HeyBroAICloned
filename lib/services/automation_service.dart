...
class AutomationService {
  ...
  final Set<String> _processedFields = {};
  ...
  final ToolsManager _toolsManager = ToolsManager();
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      print(
        '📡 Method channel handler set up for automation service singleton',
      );

      // Initialize AI client
      _aiClient = PuterClient();
      ...
      _isInitialized = true;
      print(
        '✅ AutomationService singleton initialized successfully and ready for voice commands',
      );
    } catch (e) {
  ...
  Future<void> startAutomation(String userMessage) async {
    ...
    try {
      ...
      await _channel.invokeMethod('notifyAutomationState', {
        'isAutomating': true,
      });
    } catch (_) {}
    ...
    try {
      ...
      try {
        final appsResult = await ToolsManager.executeTool(
          'get_launchable_apps',
          {},
        );
        if (appsResult['success'] == true && appsResult['data'] is List) {
          ...
          _installedApps =
              apps
                  .map(
                    (a) => (a is Map ? a['name']?.toString() ?? '' : '').trim(),
                  )
  ...
  Future<void> _executeAutomationLoop() async {
    ...
    while (_isAutomating) {
      ...
      if (!forceContextRefresh &&
          ...
          _isContextIdentical(previousContext, screenContext)) {
      ...
      if (loopResult.isLoop) {
        print(
          '🔄 LOOP DETECTED [${loopResult.loopType}]: ${loopResult.suggestion}',
        );
        if (loopResult.failedActions != null &&
            loopResult.failedActions!.isNotEmpty) {
        ...
        if (loopResult.loopType == 'exact' || loopResult.loopType == 'stall') {
          print(
            '⚠️ Severe loop detected - recovery strategies will be suggested to AI',
          );
          ...
          if (!_visionModePersistent) {
            ...
            _visionModeReason =
                'Loop recovery - accessibility may be unreliable';
      ...
      final isScrollAction =
          action != null &&
          (action.contains('scroll') || action.contains('swipe'));
  ...
  Future<Map<String, dynamic>> _captureScreenContext() async {
    try {
      ...
      final currentAppResult = await ToolsManager.executeTool(
        'get_current_app',
        {},
      );
      print(
        '🔍 currentAppResult: success=${currentAppResult['success']}, data type=${currentAppResult['data']?.runtimeType}, raw data=${currentAppResult['data']}',
      );

      Map<String, dynamic> currentApp;
      try {
        if (currentAppResult['success'] == true &&
            currentAppResult['data'] is Map) {
          currentApp = Map<String, dynamic>.from(
            currentAppResult['data'] ?? {},
          );
      ...
      final treeResult = await ToolsManager.executeTool(
        'get_accessibility_tree',
        {},
      );
      print(
        '🔍 treeResult: success=${treeResult['success']}, data type=${treeResult['data']?.runtimeType}, raw data=${treeResult['data']}',
      );
      ...
      try {
        if (treeResult['success'] == true && treeResult['data'] is List) {
          ...
          print(
            '✅ Successfully parsed accessibility tree: ${accessibilityTree.length} items',
          );
        } else {
          accessibilityTree = <dynamic>[];
          print(
            '⚠️ Accessibility tree result not a list, using empty list. Data: ${treeResult['data']}',
          );
          if (treeResult['data'] != null && treeResult['data'] is! List) {
            print(
              '🚨 CRITICAL: accessibility tree data is ${treeResult['data']?.runtimeType}, expected List',
            );
          }
        }
      } catch (e) {
      ...
      if (accessibilityTree.isNotEmpty) {
        ...
        print(
          'ℹ️ Using accessibility tree for screen elements (${screenElements.length})',
        );
      } else {
      ...
      if (_useVision) {
        screenshotResult = await ToolsManager.executeTool(
          'take_screenshot',
          {},
        );
      } else {
      ...
      print(
        '🔍 screenshotResult: success=${screenshotResult['success']}, data type=${screenshotResult['data']?.runtimeType}',
      );
      ...
      if (_showOverlay && accessibilityTree.isNotEmpty) {
        try {
          ...
          final validElements = screenElements
              .where((e) => e is Map && e['bounds'] != null)
              .toList();
          if (validElements.isNotEmpty) {
            const agentChannel = MethodChannel('com.vibeagent.dude/agent');
            await agentChannel.invokeMethod('updateA11yOverlay', {
          ...
          try {
            ...
            if (lqResult['success'] == true && lqResult['data'] is String) {
              ...
              if (isA11yEmpty) {
                ...
                print(
                  '⚠️ Vision Rescue Mode ACTIVE (Reason: Empty Accessibility Tree)',
                );
              } else {
      ...
      if (lowQualityScreenshotBase64 != null) {
        contextDimensions ??= {};
        contextDimensions['visionImageWidth'] =
            contextDimensions['ocrImageWidth'] ?? 0.0;
      }

      print(
        '📊 Context captured - App: ${currentApp['packageName'] ?? 'Unknown'}, Elements: ${screenElements.length}, Tree: ${accessibilityTree.length}',
      );
      ...
      final context = {
        ...
        'vision_fallback_active':
            visionRescueMode, // Keep for prompt compatibility (triggers Rescue Prompt)
        'vision_fallback_reason': visionRescueMode
            ? 'empty_tree'
            : (visionActive ? 'settings' : null),
      ...
      print(
        '🔍 Each field type: current_app=${context['current_app']?.runtimeType}, screen_elements=${context['screen_elements']?.runtimeType}, accessibility_tree=${context['accessibility_tree']?.runtimeType}',
      );
    ...
    for (final el in screenElements) {
      try {
        if (el is Map) {
          ...
          final className =
              map['className']?.toString() ?? map['type']?.toString() ?? '';
      ...
      if (timeDiff < 3000) {
        ...
        for (int i = 0; i < (prevTree.length < 5 ? prevTree.length : 5); i++) {
          if (i < currentTree.length) {
            final prevEl = prevTree[i] is Map
                ? Map<String, dynamic>.from(prevTree[i])
                : {};
            final currentEl = currentTree[i] is Map
                ? Map<String, dynamic>.from(currentTree[i])
                : {};
            if (prevEl['text'] != currentEl['text'] ||
                prevEl['bounds'] != currentEl['bounds']) {
              return false;
            }
          }
        }
  ...
  String _buildVisionStepPrompt(
    ...
    if (_session != null) {
      if (_session!.summary.isNotEmpty) {
        ...
        historySummary =
            "Recent actions: ${_session!.recentActions.map((s) => s.action).join(", ")}";
      }
...
''';
  ...
  String _buildStepPrompt(
    ...
    final ocrText = context['ocr_text'] is String
        ? (context['ocr_text'] as String)
        : '';
    final ocrBlocks = context['ocr_blocks'] is List
        ? List.from(context['ocr_blocks'])
        : <dynamic>[];
    final hasOcr = ocrText.isNotEmpty || ocrBlocks.isNotEmpty;
    final visionActive =
        context['vision_active'] == true ||
        context['vision_fallback_active'] == true;
...
''';
    ...
    if (loopResult?.isLoop == true) {
      final failedActionsText =
          loopResult!.failedActions != null &&
              loopResult.failedActions!.isNotEmpty
...
3. **TRY ALTERNATIVE**:
    ...
    - Look for alternative UI elements that achieve the same goal'''
              : loopResult.loopType == 'stall'
              ? '''
...
''';
  ...
  Future<Map<String, dynamic>?> _getAIDecision(
    String prompt, {
    String? image,
  }) async {
    try {
      ...
      // Check if Puter client is still authenticated
      if (_aiClient is PuterClient) {
        final puterClient = _aiClient as PuterClient;
        if (!puterClient.isSdkReady || !puterClient.isAuthenticated) {
          print('🔄 Puter client needs refresh - SDK Ready: ${puterClient.isSdkReady}, Authenticated: ${puterClient.isAuthenticated}');
          // Don't refresh here since the generateContent methods now handle this
        }
      }
      ...
      print(
        '🧠 Sending prompt to AI${image != null ? ' (with image)' : ''}...',
      );
  ...
  Future<bool> _processAIDecision(
...
''';
  ...
  bool _validateActionSequence(String action, Map<String, dynamic> parameters) {
    ...
    if (action == 'type_text') {
      ...
      final recentActions = _session != null
  ...
  String _formatOcrTextWithLineNumbers(
    List ocrBlocks, {
    double? imageWidth,
    double? imageHeight,
  }) {
    ...
    for (final block in ocrBlocks) {
      ...
      for (final line in blockLines) {
        ...
        if (bb is Map && imageHeight != null && imageHeight > 0) {
          ...
          // Vertical position
          final vPos = centerY < imageHeight / 3
              ? 'top'
              : centerY < (imageHeight * 2 / 3)
              ? 'mid'
              : 'bot';
  ...
  Map<String, dynamic>? _findBestOcrMatch(
    List ocrBlocks,
    String targetText, {
    int? occurrence,
    String? position,
    ...
    bool matchesPosition(Map? bounds) {
      ...
      if (posLower == 'middle' || posLower == 'mid') {
        if (centerY >= imgHeight / 3 && centerY < imgHeight * 2 / 3) {
          return true;
        }
      }
    ...
    for (final block in ocrBlocks) {
      ...
      if (text == target ||
          ...
          _isFuzzyMatch(text, targetText)) {
        allMatches.add({
          ...Map<String, dynamic>.from(block),
          'level': 'block',
          'matchType': text == target
              ? 'exact'
              : (text.contains(target) ? 'contains' : 'fuzzy'),
          'priority': 6,
        });
      }
    }
    ...
    for (int i = 0; i < elements.length; i++) {
      try {
        ...
        if (element is Map) {
          ...
          final contentDesc =
              elementMap['contentDescription']?.toString() ?? '';
    ...
    for (int i = 0; i < elements.length; i++) {
      try {
        ...
        if (element is Map) {
          ...
          if (_isFuzzyMatch(text, targetText) ||
              _isFuzzyMatch(contentDesc, targetText)) {
            return {
              ...elementMap,
              'index': i,
              'matched_text': text.isNotEmpty ? text : contentDesc,
  ...
  Map<String, dynamic>? _findElementByBounds(
    List elements,
    Map<String, dynamic> targetBounds,
  ) {
    for (int i = 0; i < elements.length; i++) {
      try {
        final element = elements[i];
  ...
  Future<bool> _executePreciseElementTap(
    String targetText,
    List elements,
  ) async {
    ...
    _notifyMessage(
      '📍 Precise tap at (${coords['x']!.round()}, ${coords['y']!.round()})',
    );
  ...
  Future<bool> _executePreciseElementTapByIndex(
    int index,
    List elements,
  ) async {
  ...
  Future<bool> _executePreciseElementTapByBounds(
    Map<String, dynamic> targetBounds,
    List elements,
  ) async {
  ...
  Future<bool> _executeReplayTap(Map<String, dynamic> actionData) async {
    ...
    if (actionData.containsKey('saved_coordinates')) {
      ...
      if (x != null && y != null) {
        _notifyMessage(
          '🔄 Replaying tap at saved coordinates (${x.round()}, ${y.round()})',
        );
    ...
    _notifyMessage(
      '⚠️ No saved coordinates found, using regular tap execution',
    );
  ...
  Future<bool> _executeAction(
    ...
    try {
      ...
      if (action == 'tap_vision') {
        ...
        final description =
            parameters['description'] as String? ?? 'Vision tap';
        ...
        if (w <= 0 || h <= 0) {
          ...
          final dims = await ToolsManager.executeTool(
        ...
        if (w > 0 && h > 0) {
          _notifyMessage(
            '📏 Normalizing vision tap (0-1000) to screen ${w.round()}x${h.round()}',
          );
          x = (x / 1000.0) * w;
          y = (y / 1000.0) * h;
          _notifyMessage(
            '📍 Converted to pixels: (${x.round()}, ${y.round()})',
          );
        } else {
          ...
          _notifyMessage(
            '⚠️ Could not get screen dimensions. Assuming default 1080x1920.',
          );
          w = 1080.0;
          h = 1920.0;
          x = (x / 1000.0) * w;
        ...
        _notifyMessage(
          '👁️ Vision tap: "$description" at (${x.round()}, ${y.round()})',
        );
        final result = await ToolsManager.executeTool('perform_tap', {
          'x': x,
          'y': y,
      ...
      if (action.startsWith('tap_element_by_text') ||
          action == 'find_and_click') {
        ...
        if (!success) {
          print(
            '⚠️ Accessibility tap failed for "$targetText". Trying OCR/Vision fallback...',
          );
          _notifyMessage(
            '👁️ Accessibility tap failed. Trying Vision match...',
          );
      ...
      if (action == 'tap_element_by_bounds') {
        ...
        if (!bounds.containsKey('left') ||
            !bounds.containsKey('top') ||
            !bounds.containsKey('right') ||
            !bounds.containsKey('bottom')) {
      ...
      if (action == 'tap_ocr_line') {
        ...
        List ocrBlocks = (ctx['ocr_blocks'] is List)
            ? List.from(ctx['ocr_blocks'])
            : <dynamic>[];
        ...
        _notifyMessage(
          '🎯 OCR line tap [L$lineNum] "$text" at (${coords['x']!.round()}, ${coords['y']!.round()})',
        );
        final result = await ToolsManager.executeTool('perform_tap', {
          'x': coords['x'],
          'y': coords['y'],
      ...
      if (action == 'tap_ocr_text') {
        ...
        if (block == null) {
          _notifyMessage(
            '❌ No matching OCR block for: "$text"${occurrence != null ? " (occurrence $occurrence)" : ""}',
          );
          return false;
        }
        ...
        final coords = _getCenterFromBounds(bounds);
        _notifyMessage(
          '🎯 OCR tap [$level/$matchType] on "$matchedText" at (${coords['x']!.round()}, ${coords['y']!.round()})',
        );
        final result = await ToolsManager.executeTool('perform_tap', {
          'x': coords['x'],
          'y': coords['y'],
      ...
      if (action == 'tap_ocr_bounds') {
        final hasAll =
            parameters.containsKey('left') &&
            parameters.containsKey('top') &&
            parameters.containsKey('right') &&
            parameters.containsKey('bottom');
        if (!hasAll) {
        ...
        _notifyMessage(
          '📍 OCR bounds tap at (${coords['x']!.round()}, ${coords['y']!.round()})',
        );
        final result = await ToolsManager.executeTool('perform_tap', {
          'x': coords['x'],
          'y': coords['y'],
      ...
      if (action == 'perform_swipe' &&
          parameters.containsKey('direction') &&
          !parameters.containsKey('startX')) {
        final direction =
            parameters['direction']?.toString().toLowerCase() ?? 'left';
        _notifyMessage(
          '🔄 converting directional swipe "$direction" to coordinates',
        );

        // Get screen dimensions
        final w =
            (_lastContext?['ocr_image_width'] as num?)?.toDouble() ??
            (_lastContext?['vision_image_width'] as num?)?.toDouble() ??
            1080.0;
        final h =
            (_lastContext?['ocr_image_height'] as num?)?.toDouble() ?? 2400.0;
      ...
      if (action == 'perform_internal_scroll') {
        final direction =
            parameters['direction']?.toString().toLowerCase() ?? 'down';
        final index = parameters['index'] as int?;
        final text = parameters['text'] as String?;
        ...
        _notifyMessage(
          '🔄 Scroll "$direction" inside element [${targetElement['index']}]',
        );
        _notifyMessage(
          '📍 Swipe: (${startX.round()},${startY.round()}) -> (${endX.round()},${endY.round()})',
        );
      ...
      if ((action == 'type_text' || action == 'advanced_type_text') &&
        ...
        try {
          ...
          if (textToType.isNotEmpty) {
            ...
            final robustResult = await ToolsManager.executeTool(
              'robust_text_input',
              {'text': textToType, 'maxRetries': 3},
            );
          ...
          _notifyMessage(
            '⚠️ Robust text input error: $e, falling back to normal typing',
          );
        }
      }
  ...
  String _formatElements(List elements) {
    ...
    final buffer = StringBuffer();
    buffer.writeln(
      'INTERACTIVE ELEMENTS (use index numbers for tap_element_by_index):',
    );
        ...
        if (element is Map) {
          try {
            ...
            final contentDesc =
                elementMap['contentDescription']?.toString() ?? '';
            final type =
                elementMap['className']?.toString() ??
                elementMap['type']?.toString() ??
                'Unknown';
            final bounds = elementMap['bounds'];
            final clickable = elementMap['clickable'] == true
                ? '✓ CLICKABLE'
                : '✗ Not clickable';
            ...
            buffer.writeln(
              '[$i] $type: $content | $clickable $scrollable | $boundsInfo',
            );
          } catch (e) {
            print('Error processing element $i: $e');
  ...
  String _formatAccessibilityTree(List tree) {
    ...
    final buffer = StringBuffer();
    buffer.writeln(
      'ACCESSIBILITY TREE (use index numbers for tap_element_by_index):',
    );
        ...
        if (node is Map) {
          try {
            ...
            final contentDescription =
                nodeMap['contentDescription']?.toString() ?? '';
            final className =
                nodeMap['className']?.toString() ??
                nodeMap['type']?.toString() ??
                'Node';
            final clickable = nodeMap['clickable'] == true
                ? '✓ CLICKABLE'
                : '✗ Not clickable';
            ...
            if (text.isNotEmpty ||
                ...
                nodeMap['clickable'] == true ||
                nodeMap['scrollable'] == true ||
                nodeMap['editable'] == true) {
              ...
              if (bounds != null && bounds is Map) {
                ...
                final centerY =
                    ...
                    ((top + bottom) / 2).round();
                boundsInfo =
                    ' | bounds(L:$left,T:$top,R:$right,B:$bottom) center($centerX,$centerY)';
              }
              ...
              buffer.writeln(
                '[$i] $className: $content | $clickable $scrollable $editable$boundsInfo',
              );
            }
          } catch (e) {
  ...
  double _textMatchScore(String text, String target) {
    ...
    return (score * 0.7) +
        ((prefix / (target.isEmpty ? 1 : target.length)) * 0.3);
  }
  ...
  Future<void> _notifyAndroidCompletion(bool success, [String? error]) async {
    try {
      if (success) {
        ...
        await _channel.invokeMethod('notifyAutomationError', {
  ...
  Future<void> _ensureInputFocusBeforeTyping(
    String intendedText, {
    bool force = false,
  }) async {
    try {
      ...
      if (a11y is List && a11y.isNotEmpty) {
        ...
        if (editableIdx != -1) {
          ...
          if (!_processedFields.contains(fieldId)) {
            ...
            await _executeAction('tap_element_by_index', {
              'index': editableIdx,
            });
      ...
      List blocks = (context['ocr_blocks'] is List)
          ? List.from(context['ocr_blocks'])
          : <dynamic>[];
      if (blocks.isEmpty &&
          context['screenshot_available'] == true &&
          ((a11y is! List) || a11y.isEmpty || _lastContextLooksLikeWeb())) {
        final ss = await ToolsManager.executeTool('take_screenshot', {});
        if (ss['success'] == true && ss['data'] is String) {
          final ocrRes = await ToolsManager.executeTool('perform_ocr', {
            'screenshot': ss['data'],
      ...
      if (best != null) {
        ...
        if (!_processedFields.contains(fieldId)) {
          ...
          _notifyMessage(
            '🖱️ Focusing input via OCR at (${center['x']!.round()}, ${center['y']!.round()})',
          );
          await ToolsManager.executeTool('perform_tap', {
            'x': center['x'],
            'y': center['y'],
          });
          ...
          _notifyMessage(
            '⏭️ Skipping already processed OCR field at (${center['x']!.round()}, ${center['y']!.round()})',
          );
        }
      }
    } catch (_) {}
  ...
  Future<void> _visionModeTapInputField() async {
    try {
      ...
      final ctx = _lastContext ?? <String, dynamic>{};
      List blocks = (ctx['ocr_blocks'] is List)
          ? List.from(ctx['ocr_blocks'])
          : <dynamic>[];
      ...
      if (bestMatch == null) {
        ...
        for (final block in blocks) {
          ...
          if (text.contains('...') ||
              ...
              text.toLowerCase().contains('type')) {
      ...
      if (bestMatch != null) {
        ...
        _notifyMessage(
          '👆 Vision tap on input: "$matchedText" [$level] at (${center['x']!.round()}, ${center['y']!.round()})',
        );
        await ToolsManager.executeTool('perform_tap', {
          'x': center['x'],
          'y': center['y'],
    ...
    for (int i = 0; i < a11y.length; i++) {
      try {
        ...
        if (node is Map) {
          ...
          if (m['editable'] == true ||
              (m['className']?.toString().toLowerCase().contains('edittext') ??
                  false)) {
            return i;
          }
        }
  ...
  /// Detect system dialogs in accessibility tree
  List<Map<String, dynamic>> _detectSystemDialogs(
    List<dynamic> accessibilityTree,
    Map<String, dynamic> currentApp,
  ) {
  ...
  String _formatSystemDialogs(List<Map<String, dynamic>> systemDialogs) {
    ...
    for (final dialog in systemDialogs) {
      ...
      final clickable = dialog['clickable'] == true
          ? 'CLICKABLE'
          : 'Not clickable';
      ...
      if (contentDesc.isNotEmpty) {
        buffer.writeln('    Description: "$contentDesc"');
      }
  ...
  Future<bool> performDynamicScroll({
    ...
    try {
      print(
        '🔄 Starting dynamic scroll: direction=$direction, target="$targetText"',
      );
      ...
      for (
        int attempt = 1;
        attempt <= maxScrollAttempts && !targetFound;
        attempt++
      ) {
        ...
        if (previousSnapshots.isNotEmpty &&
          ...
          print(
            '⚠️ Identical snapshot detected ($consecutiveIdenticalCount/$consecutiveIdenticalThreshold)',
          );

          if (consecutiveIdenticalCount >= consecutiveIdenticalThreshold) {
            print(
              '🛑 Reached end of scrollable content in $direction direction',
            );
      ...
      print(
        '❌ Target "$targetText" not found after $maxScrollAttempts scroll attempts',
      );
      return false;
    } catch (e) {
  ...
  bool _fuzzyMatchTarget(Map<String, dynamic> context, String targetText) {
    try {
      ...
      final matchRatio = targetWords.isNotEmpty
          ? matchedWords / targetWords.length
          : 0.0;
