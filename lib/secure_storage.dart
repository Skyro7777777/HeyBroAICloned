...
class SecureStorage {
  ...
  static const String _automationModeKey =
      'automation_mode'; // 'vision_a11y' (default) or 'a11y_only'
  static const String _a11yOverlayKey =
      'a11y_overlay_enabled'; // 'true' or 'false'

  // Puter.js specific keys
  static const String _puterAuthTokenKey = 'puter_auth_token';
  static const String _puterModelIdKey = 'puter_model_id';
  ...
  /// Available Puter.js AI models (free via user's Puter account)
  static const Map<String, Map<String, String>> puterAvailableModels = {
    'openai/gpt-5.4-nano': {
      'name': 'GPT-5.4 Nano',
      'provider': 'OpenAI',
      'capabilities': 'Text only, cheapest, fast',
      'pricing': '\$0.20/\$1.25 per M tokens',
      'vision': 'false',
      'recommended': 'true',
    },
    'openai/gpt-5.4-mini': {
      'name': 'GPT-5.4 Mini',
      'provider': 'OpenAI',
      'capabilities': 'Text + Vision, fast, efficient',
      'pricing': '\$0.75/\$4.50 per M tokens',
      'vision': 'true',
      'recommended': 'false',
    },
    'openai/gpt-5.4': {
      'name': 'GPT-5.4',
      'provider': 'OpenAI',
      'capabilities': 'Frontier model, complex tasks',
    ...
    'google/gemini-3-flash-preview': {
      'name': 'Gemini 3 Flash',
      'provider': 'Google',
      'capabilities': 'Fast reasoning, vision, coding',
      'pricing': '\$0.50/\$3 per M tokens',
      'vision': 'true',
      'recommended': 'true',
    },
    'google/gemini-2.5-flash-lite': {
      'name': 'Gemini 2.5 Flash Lite',
      'provider': 'Google',
      'capabilities': 'Lightweight, cost-optimized',
    ...
    'anthropic/claude-sonnet-4-6': {
      ...
      'provider': 'Anthropic',
      'capabilities': 'Excellent for agents & computer use',
      'pricing': '\$3.00/\$15 per M tokens',
      'vision': 'true',
      'recommended': 'false',
    },
    'deepseek/deepseek-chat-v3.1': {
      'name': 'DeepSeek V3.1',
      'provider': 'DeepSeek',
      'capabilities': 'Thinking + non-thinking modes',
      'pricing': '\$0.15/\$0.75 per M tokens',
      'vision': 'false',
      'recommended': 'false',
    },
    'x-ai/grok-4-1-fast': {
      'name': 'Grok 4.1 Fast',
      'provider': 'xAI',
      'capabilities': 'Fast tool-calling, web search',
    ...
    'qwen/qwen3.5-flash-02-23': {
      'name': 'Qwen 3.5 Flash',
      'provider': 'Qwen',
      'capabilities': 'Ultra-cheap, efficient agent',
      'pricing': '\$0.07/\$0.26 per M tokens',
      'vision': 'true',
      'recommended': 'false',
    },
    // Free models
    'qwen/qwen3.6-plus-preview:free': {
      ...
      'provider': 'Qwen',
      'capabilities': 'Free, 1M context, strong reasoning',
      'pricing': 'FREE',
      'vision': 'false',
      'recommended': 'false',
    },
    'arcee-ai/trinity-large-preview:free': {
      ...
      'provider': 'Arcee AI',
      'capabilities': 'Free, 128K context, agentic',
      'pricing': 'FREE',
      'vision': 'false',
      'recommended': 'false',
    },
    ...
    try {
      ...
      if (serviceAccountJson != null && serviceAccountJson.isNotEmpty) {
        if (!_isValidServiceAccount(serviceAccountJson)) {
          ...
          if (!_isValidServiceAccount(serviceAccountJson)) {
            throw ArgumentError(
                'Invalid Service Account JSON format ($serviceAccountJson)');
          }
        }
      }
      ...
      final modelConfig = {
        ...
        'model_name': availableModels[modelId]?['name'] ??
            modelId, // Safe lookup with fallback
        'region': region,
    ...
    try {
      ...
      return ApiConfiguration(
        ...
        modelId: modelConfig['model_id'] ??
            'gemini-2.0-flash-exp', // Updated default
  ...
  Future<void> saveA11yOverlayEnabled(bool enabled) async {
    try {
      await _secureStorage.write(
          key: _a11yOverlayKey, value: enabled.toString());
    } catch (e) {
      rethrow;
    }
  }
  ...
  /// Save Puter auth token
  Future<void> savePuterAuthToken(String token) async {
    try {
      final encrypted = await _encryptData(token);
      await _secureStorage.write(key: _puterAuthTokenKey, value: encrypted);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<String?> getPuterAuthToken() async {
    try {
      final encrypted = await _secureStorage.read(key: _puterAuthTokenKey);
  ...
  /// Save Puter model ID
  Future<void> savePuterModelId(String modelId) async {
    try {
      await _secureStorage.write(key: _puterModelIdKey, value: modelId);
    } catch (e) {
      rethrow;
    }
  }
  ...
  /// Get Puter model ID
  Future<String?> getPuterModelId() async {
    try {
      return await _secureStorage.read(key: _puterModelIdKey);
    } catch (e) {
      return null;
    }
  }

  /// Check if Puter is configured (has auth token)
  Future<bool> hasPuterConfiguration() async {
    final token = await getPuterAuthToken();
    return token != null && token.isNotEmpty;
  }

  /// Clear Puter configuration
  Future<void> clearPuterConfiguration() async {
    try {
      await _secureStorage.delete(key: _puterAuthTokenKey);
      await _secureStorage.delete(key: _puterModelIdKey);
    } catch (e) {
      rethrow;
    }
  }
