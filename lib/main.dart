...
class _HomeWrapperState extends State<HomeWrapper> {
  ...
  Future<void> _checkPermissions() async {
    try {
      ...
      // We check if user has previously signed in (marker stored in SecureStorage)
      final secureStorage = SecureStorage();
      final puterAuth = await secureStorage.getPuterAuthToken();
      final apiConfigured = puterAuth != null && puterAuth.isNotEmpty;
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Stack(
        children: [
          const HomeWrapper(),
          // Hidden WebView for Puter.js AI calls (must be in widget tree)
          PuterClient().buildHiddenWebView(),
        ],
      ),
