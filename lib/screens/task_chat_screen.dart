  ...
  Future<void> _performAutomation() async {
    try {
      ...
      _automationService.onLog = (type, content) async {
        await _taskService.addLogToTask(
            _currentTask!.id, TaskLog(type: type, content: content));
        _currentTask = _taskService.getTask(_currentTask!.id);
      };
      ...
      _automationService.onMessage = (message) async {
        ...
        try {
          ...
          if (decoded is Map<String, dynamic>) {
            ...
            if (decoded.containsKey('task_completed') &&
            ...
            if (decoded.containsKey('tool_calls') ||
                decoded.containsKey('function_call') ||
                decoded.containsKey('action')) {
              // For tap_element_by_index actions, add saved coordinates from automation service
              if (decoded['action'] == 'tap_element_by_index' &&
                  _automationService.lastTapCoordinates != null) {
                decoded['saved_coordinates'] =
                    _automationService.lastTapCoordinates;
      ...
      if (_useForegroundService) {
        final success =
            await ForegroundAutomationService.startForegroundAutomation(
        ...
        if (success) {
          ...
          final foregroundMessage = ChatMessage(
            text:
                "🚀 Task started in background service - it will continue even if you close the app!",
            isUser: false,
          );
          await _taskService.addMessageToTask(
              _currentTask!.id, foregroundMessage);
          _currentTask = _taskService.getTask(_currentTask!.id);
          if (mounted) {
            setState(() {});
            _scrollToBottom();
    ...
    try {
      ...
      for (int i = 0; i < actionsToReplay.length; i++) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('🔧 Executing action ${i + 1}/${actionsToReplay.length}'),
