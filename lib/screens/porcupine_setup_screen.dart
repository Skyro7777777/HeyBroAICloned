...
class PorcupineSetupScreen extends StatefulWidget {
  const PorcupineSetupScreen({super.key});
  ...
  Future<void> _testApiKey() async {
    ...
    try {
      ...
      final result = await platform
          .invokeMethod('testPorcupineKey', {'accessKey': apiKey});
        ...
        if (isSuccess) {
          ...
          if (result is Map && result.containsKey('error')) {
            ...
            if (error.contains('00000136')) {
              message =
                  'Invalid API key: Authentication failed. Please check your Picovoice Console for the correct key.';
      ...
      if (e.toString().contains('ACCESS_KEY_TEST_ERROR')) {
        errorMessage =
            'API key validation failed: Please check your internet connection and try again';
